...
const viewer=$('#viewer');if(viewer){
  const titleRow=$('.viewer-title-row',viewer),page=$('#viewerPage');
  ...
  const activeIndex=()=>Math.max(0,$$('.thumb','#thumbStrip').findIndex(x=>x.classList.contains('active')));
  ...
  function schedule(){clearTimeout(timer);if(!playing)return;animateCurrent();timer=setTimeout(()=>{if(!playing)return;const thumbs=$$('.thumb','#thumbStrip');const i=activeIndex();if(i<0||!thumbs.length){setPlaying(false);return;}if(i>=thumbs.length-1){setPlaying(false);status.textContent='Finished';status.classList.add('show');setTimeout(()=>{status.classList.remove('show');status.textContent='Presentation playback';},1300);return;}thumbs[i+1].click();setTimeout(schedule,80);},3200);}
  function start(){if(isPdf())return;const thumbs=$$('.thumb','#thumbStrip');if(!thumbs.length)return;if(activeIndex()>=thumbs.length-1)thumbs[0].click();setPlaying(true);schedule();}
  ...
  const obs=new MutationObserver(()=>{
    const open=viewer.classList.contains('open');const pdf=isPdf();play.classList.toggle('hidden',!open||pdf);if((!open||pdf)&&playing)setPlaying(false);
